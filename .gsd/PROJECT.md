...
The core package model is built: `tool.Package` defines static tool/package types, `toolset.Builder` assembles request-scoped toolsets from `AddFromDir` (local source), `AddFromArchive` (built .toolbox.pkg), and now `AddFromRegistry` (registry resolution). `packaging.Pack` produces archives with sha256 integrity, `packaging.LoadArchive` verifies and loads them. The `registry/` package includes a cache layout, a GitHub Releases source, a git-source fallback, pseudo-version git resolution, a `Resolver` that orchestrates cache-first resolution with ordered source fallback, and version discovery for both GitHub releases and git tags. Declarative `toolbox.toolset.json` loading, lockfiles, and local replace overlays now live in the dedicated `toolsetfile/` package. The unified `cmd/toolbox` entrypoint is closed on the shipped M001 surface: `toolbox resolve`, `toolbox versions`, single-module `toolbox resolve --upgrade`, and `--file` selection are all proven through focused CLI-boundary tests, including committed-lock cache-hit re-resolve, sibling local-overlay behavior, token-auth header wiring without token leakage, and CLI-local validation failures. S12 is complete; M001 is now at milestone closeout/validation rather than further feature assembly.
